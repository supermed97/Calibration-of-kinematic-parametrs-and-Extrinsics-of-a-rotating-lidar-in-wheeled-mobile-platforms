...
int main(int argc, char** argv) {
  ...
  std::vector<float> x_;      // work vector 3
  std::vector<float> y_;      // work vector 3
  std::vector<float> theta_;      // work vector 3
  ...
  while (std::getline(input_File, line)) {
    ...
    if (ss >> time_stamp >> x >> y >> theta >> beam_angle >> range >> x_endpoint >> y_endpoint) {
      ...
      if (p(0)-x_endpoint >= 0.1 || p(0)-x_endpoint<=-0.1  ){
        cerr <<"__in if block__"<<endl;
        cerr << " __ x_endpoint__ : " << x_endpoint << endl;
        cerr << " __ y_endpoint__ :  " << y_endpoint << endl;
        cerr << "__P__ :  " << p.transpose() << endl; 
        Eigen::Vector2f p_negative(-range*cos(beam_angle), -range*sin(beam_angle));
        p=iso*p_negative;
        cerr << "__P__ after:  " << p.transpose() << endl; 
        cerr <<"__out of if block__"<<endl;
        neg_cnt++;
		   } 
      os << p.transpose() << endl;
    }
