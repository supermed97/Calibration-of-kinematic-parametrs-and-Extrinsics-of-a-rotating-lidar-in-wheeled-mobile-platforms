...
while(end_time <= sim_time){
	pose.theta= delta_theta(2, omega_r , omega_l, end_time,b);
	...
	cout<< "I AM POINTTTT" << point_intersect <<endl;
	Filewrite_for_micp(micp_file, end_time, &pose ,  beam_angle , range , point_intersect, min_scalar );   // write to file for micp  }
